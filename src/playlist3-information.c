...
static void info3_fill_song_view(char *);
...
static void as_album_viewed_clicked(GtkButton *button, gpointer data)
{
	...
	info3_fill_album_view(artist,album);
	g_free(artist);
	g_free(album);
	info3_type = INFO3_TYPE_NONE;
}
...
GtkWidget *info3_create_artist_button(mpd_Song *song)
{
	...
	gtk_widget_set_app_paintable(GTK_WIDGET(event), TRUE);
	g_signal_connect(G_OBJECT(event), "expose-event", G_CALLBACK(info3_row_expose_event), NULL);
 ...
 * Song View 
 */
static void info3_fill_song_view(char *path) 
{
	GtkWidget *ali = NULL;
	GtkWidget *vbox= NULL;
	mpd_Song *song = NULL;
	 ...
	 * Clear header
	 */
	info3_widget_clear_children(title_vbox);
	...
	gtk_button_set_relief(GTK_BUTTON(button), GTK_RELIEF_NONE);
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(info3_fill_view), NULL);
...
static void info3_show_current_song()
{
	mpd_Song *song = mpd_playlist_get_current_song(connection);
	if(song)
	{
		info3_fill_song_view(song->file);	
		info3_type = INFO3_TYPE_CURRENT_SONG;
	}
}
 ...
 * Collection view
 */
static void info3_fill_view_entry_activate(GtkEntry *entry, GtkWidget *table)
{
	regex_t regt;
	...
	if(strlen(text) && !regcomp(&regt, text, REG_EXTENDED|REG_ICASE|REG_NOSUB))
	{
		...
		for(;data;data = mpd_data_get_next(data))
		{
			...
			if(!regexec(&regt,data->tag, 0,NULL,0))
			{
				...
				/* Create button */
				button = info3_create_artist_button(song);
...
/*
	...
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(info3_show_current_artist), NULL);
	gtk_box_pack_start(GTK_BOX(hbox), ali, FALSE, TRUE, 0);
	...
	ali = gtk_alignment_new(0,0.5,0,0);
	gtk_container_add(GTK_CONTAINER(ali), button);                                             	
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(info3_show_current_album), NULL);
	gtk_box_pack_start(GTK_BOX(hbox), ali, FALSE, TRUE, 0);

	button = gtk_button_new_with_label("View Song");
	ali = gtk_alignment_new(0,0.5,0,0);
	gtk_container_add(GTK_CONTAINER(ali), button);                                             	
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(info3_show_current_song), NULL);
	gtk_box_pack_start(GTK_BOX(hbox), ali, FALSE, TRUE, 0);
...
*/
	...
	g_signal_connect(G_OBJECT(entry), "activate", G_CALLBACK(info3_fill_view_entry_activate), artist_table);
	/**
	 * Entry completion
	 */
...
static void info3_fill_artist_view(char *artist)
{
	GtkWidget *ali = NULL;
	mpd_Song *song2 = NULL;
	PassData *pd = NULL;
	...
	meta_data_get_path_callback(song2, META_ARTIST_TXT, (MetaDataCallback)info3_cover_txt_fetched, pd);
	gtk_box_pack_start(GTK_BOX(resizer_vbox), vbox, FALSE, FALSE,0);
	...
	if(song2 && song2->artist)
	{
		...
		for(;data;data = mpd_data_get_next(data))
		{
			...
			if(tracks)
			{
				...
				info3_add_table_item(table2, _("<b>Tracks:</b>"), str,i);
				g_free(str);
				i++;
			}
			if(song->file)
			{
				...
				info3_add_table_item(table2,_("<b>Directory:</b>"),dirname,i);
				i++;
				g_free(dirname);
			}
 ...
 * View Album
 */
static void info3_fill_album_view(char *artist,char *album)
{
	GtkWidget *button = NULL;
	GtkWidget *ali = NULL;
	mpd_Song *song2 = NULL;
	...
	meta_data_get_path_callback(song2, META_ALBUM_TXT, (MetaDataCallback)info3_cover_txt_fetched, pd);
	gtk_box_pack_start(GTK_BOX(resizer_vbox), vbox, FALSE, FALSE,0);
	...
	if(song2 && song2->artist)
	{
		...
		/** Album name */
		if(song2->album){
			info3_add_table_item(table2, _("<b>Album:</b>"), song2->album, i);
			i++;
		}
		/**
		 * Do Genre Search 
		...
		if(string->len >0)
		{
			info3_add_table_item(table2, _("<b>Genre:</b>"), string->str, i);
			i++;
		}
		...
		if(data)
		{
			...
			if(song->file) {
				...
				info3_add_table_item(table2, _("<b>Directory:</b>"), filename, i);
				g_free(filename);
				i++;
			}
		}
