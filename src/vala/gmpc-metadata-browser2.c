...
static void gmpc_widget_more_size_changed (GmpcWidgetMore* self, GtkWidget* child, GdkRectangle* alloc);
...
static void _gmpc_widget_more_size_changed_gtk_widget_size_allocate (GtkWidget* _sender, GdkRectangle* allocation, gpointer self);
...
static void _lambda0_ (GtkExpander* source, Block2Data* _data2_);
static void __lambda0__gtk_expander_activate (GtkExpander* _sender, gpointer self);
...
static void _lambda7_ (GtkToggleButton* source, GmpcMetadataBrowser* self);
			...
			if (list != NULL) {
				...
				if (data != NULL) {
					...
					if (g_utf8_validate (iter->tag, -1, NULL) == FALSE) {
						g_error ("gmpc-metadata-browser2.vala:344: Failed to validate");
					}
					if (iter->tag != NULL) {
						char* _tmp2_;
...
static void _lambda0_ (GtkExpander* source, Block2Data* _data2_) {
	...
	if (!_data2_->simsong_showed) {
		GmpcWidgetSimilarSongs* similar_songs;
		g_debug ("gmpc-metadata-browser2.vala:1588: Update\n");
		similar_songs = g_object_ref_sink (gmpc_widget_similar_songs_new (_data1_->song));
		gtk_container_add ((GtkContainer*) _data2_->label2, (GtkWidget*) similar_songs);
		gmpc_widget_similar_songs_update (similar_songs);
		_data2_->simsong_showed = TRUE;
...
GtkWidget* gmpc_metadata_browser_metadata_box_show_song (GmpcMetadataBrowser* self, const mpd_Song* song, gboolean show_controls) {
	...
	_data1_->song = _mpd_songDup0 (song);
	...
	if (_data1_->song->title != NULL) {
		char* _tmp0_;
		gtk_label_set_markup (label, _tmp0_ = g_markup_printf_escaped ("<span size='xx-large' weight='bold'>%s</span>", _data1_->song->title));
		_g_free0 (_tmp0_);
	} else {
		char* _tmp1_;
	...
	if (_data1_->song->title != NULL) {
		...
		g_object_set_data_full ((GObject*) button, "query", (void*) g_strdup_printf ("title=(%s)", _data1_->song->title), (GDestroyNotify) g_free);
		...
		gmpc_metadata_browser_add_entry (self, info_box, _ ("Title"), _data1_->song->title, (GtkWidget*) button, &i);
	...
	if (_data1_->song->artist != NULL) {
		...
		gtk_button_set_relief (button, GTK_RELIEF_NONE);
		g_object_set_data_full ((GObject*) button, "artist", (void*) g_strdup_printf ("%s", _data1_->song->artist), (GDestroyNotify) g_free);
		g_signal_connect_object (button, "clicked", (GCallback) _gmpc_metadata_browser_artist_button_clicked_gtk_button_clicked, self, 0);
		gmpc_metadata_browser_add_entry (self, info_box, _ ("Artist"), _data1_->song->artist, (GtkWidget*) button, &i);
		_g_object_unref0 (button);
	}
	gmpc_metadata_browser_add_entry (self, info_box, _ ("Album artist"), _data1_->song->albumartist, NULL, &i);
	if (_data1_->song->album != NULL) {
		...
		if (_data1_->song->artist != NULL) {
			...
			gtk_button_set_relief (button, GTK_RELIEF_NONE);
			g_object_set_data_full ((GObject*) button, "artist", (void*) g_strdup_printf ("%s", _data1_->song->artist), (GDestroyNotify) g_free);
			g_object_set_data_full ((GObject*) button, "album", (void*) g_strdup_printf ("%s", _data1_->song->album), (GDestroyNotify) g_free);
			g_signal_connect_object (button, "clicked", (GCallback) _gmpc_metadata_browser_album_button_clicked_gtk_button_clicked, self, 0);
		}
		gmpc_metadata_browser_add_entry (self, info_box, _ ("Album"), _data1_->song->album, (GtkWidget*) button, &i);
		_g_object_unref0 (button);
	}
	gmpc_metadata_browser_add_entry (self, info_box, _ ("Track"), _data1_->song->track, NULL, &i);
	gmpc_metadata_browser_add_entry (self, info_box, _ ("Date"), _data1_->song->date, NULL, &i);
	gmpc_metadata_browser_add_entry (self, info_box, _ ("Performer"), _data1_->song->performer, NULL, &i);
	gmpc_metadata_browser_add_entry (self, info_box, _ ("Disc"), _data1_->song->disc, NULL, &i);
	gmpc_metadata_browser_add_entry (self, info_box, _ ("Genre"), _data1_->song->genre, NULL, &i);
	if (_data1_->song->file != NULL) {
		...
		_g_object_unref0 (_tmp7_);
		g_object_set_data_full ((GObject*) dbutton, "path", (void*) g_path_get_dirname (_data1_->song->file), (GDestroyNotify) g_free);
		...
		gmpc_metadata_browser_add_entry (self, info_box, _ ("Path"), _data1_->song->file, (GtkWidget*) dbutton, &i);
	...
	gmpc_metadata_browser_add_entry (self, info_box, _ ("Comment"), _data1_->song->comment, NULL, &i);
	...
	if (show_controls) {
		...
		gtk_button_set_relief (button, GTK_RELIEF_NONE);
		g_object_set_data_full ((GObject*) button, "song", mpd_songDup (_data1_->song), (GDestroyNotify) mpd_freeSong);
	...
	if (cfg_get_single_value_as_int_with_default (config, "MetaData", "show-similar-songs", 1) == 1) {
		...
		_data2_->simsong_showed = FALSE;
		_data2_->label2 = (_tmp19_ = g_object_ref_sink ((GtkExpander*) gtk_expander_new (_tmp18_ = g_markup_printf_escaped ("<b>%s</b>", _ ("Similar Songs")))), _g_free0 (_tmp18_), _tmp19_);
		gtk_expander_set_use_markup (_data2_->label2, TRUE);
		gtk_box_pack_start ((GtkBox*) vbox, (GtkWidget*) _data2_->label2, FALSE, FALSE, (guint) 0);
		g_signal_connect_data (_data2_->label2, "activate", (GCallback) __lambda0__gtk_expander_activate, block2_data_ref (_data2_), (GClosureNotify) block2_data_unref, 0);
		gtk_widget_show ((GtkWidget*) _data2_->label2);
...
static void gmpc_metadata_browser_metadata_box_show_artist (GmpcMetadataBrowser* self, const char* artist) {
	...
	if (cfg_get_single_value_as_int_with_default (config, "MetaData", "show-similar-artist", 1) == 1) {
		...
		similar_artist = g_object_ref_sink (gmpc_widget_similar_artist_new (connection, song));
...
static void gmpc_metadata_browser_real_preferences_pane_construct (GmpcPluginPreferencesIface* base, GtkContainer* container) {
	...
	gtk_box_pack_start ((GtkBox*) box, (GtkWidget*) chk, FALSE, FALSE, (guint) 0);
	g_signal_connect_object ((GtkToggleButton*) chk, "toggled", (GCallback) __lambda7__gtk_toggle_button_toggled, self, 0);
