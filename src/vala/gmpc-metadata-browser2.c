...
GtkWidget* gmpc_metadata_browser_metadata_box_show_song (GmpcMetadataBrowser* self, const mpd_Song* song) {
	...
	/* Artist label */
	gmpc_metadata_browser_add_entry (self, info_box, _ ("Artist"), song->artist, NULL, &i);
	/* AlbumArtist label */
	gmpc_metadata_browser_add_entry (self, info_box, _ ("Album artist"), song->albumartist, NULL, &i);
	/* Album */
	gmpc_metadata_browser_add_entry (self, info_box, _ ("Album"), song->album, NULL, &i);
	/* track */
	...
	if (song->file != NULL) {
		...
		_tmp3_ = NULL;
		gtk_container_add ((GtkContainer*) dbutton, (GtkWidget*) (_tmp3_ = g_object_ref_sink ((GtkImage*) gtk_image_new_from_stock ("gtk-open", GTK_ICON_SIZE_MENU))));
	...
	_tmp4_ = NULL;
	ali = (_tmp4_ = g_object_ref_sink ((GtkAlignment*) gtk_alignment_new (0.0f, 0.5f, 0.f, 0.f)), (ali == NULL) ? NULL : (ali = (g_object_unref (ali), NULL)), _tmp4_);
	...
	if (mpd_sticker_supported (connection)) {
		...
		_tmp5_ = NULL;
		ali = (_tmp5_ = g_object_ref_sink ((GtkAlignment*) gtk_alignment_new (0.0f, 0.5f, 0.f, 0.f)), (ali == NULL) ? NULL : (ali = (g_object_unref (ali), NULL)), _tmp5_);
	...
	_tmp6_ = NULL;
	hbox = (_tmp6_ = g_object_ref_sink ((GtkHBox*) gtk_hbox_new (FALSE, 6)), (hbox == NULL) ? NULL : (hbox = (g_object_unref (hbox), NULL)), _tmp6_);
	gtk_box_pack_start ((GtkBox*) hbox, (GtkWidget*) button, FALSE, FALSE, (guint) 0);
	_tmp7_ = NULL;
	button = (_tmp7_ = g_object_ref_sink ((GtkButton*) gtk_button_new_from_stock ("gtk-add")), (button == NULL) ? NULL : (button = (g_object_unref (button), NULL)), _tmp7_);
	gtk_button_set_relief (button, GTK_RELIEF_NONE);
	g_object_set_data_full ((GObject*) button, "song", mpd_songDup (song), (GDestroyNotify) mpd_freeSong);
	...
	/**
	        ...
	        */
	_tmp14_ = NULL;
	return (_tmp14_ = (GtkWidget*) vbox, (box == NULL) ? NULL : (box = (g_object_unref (box), NULL)), (label == NULL) ? NULL : (label = (g_object_unref (label), NULL)), (hbox == NULL) ? NULL : (hbox = (g_object_unref (hbox), NULL)), (ali == NULL) ? NULL : (ali = (g_object_unref (ali), NULL)), (artist_image == NULL) ? NULL : (artist_image = (g_object_unref (artist_image), NULL)), (info_box == NULL) ? NULL : (info_box = (g_object_unref (info_box), NULL)), (fav_button == NULL) ? NULL : (fav_button = (g_object_unref (fav_button), NULL)), (button == NULL) ? NULL : (button = (g_object_unref (button), NULL)), _tmp14_);
}
...
static void gmpc_metadata_browser_metadata_box_show_album (GmpcMetadataBrowser* self, const char* artist, const char* album) {
	...
	/* Dates of songs */
	_tmp7_ = NULL;
	pt_label = (_tmp7_ = g_object_ref_sink (gmpc_stats_label_new (ALBUM_DATES_SONGS, song)), (pt_label == NULL) ? NULL : (pt_label = (g_object_unref (pt_label), NULL)), _tmp7_);
	gtk_label_set_line_wrap ((GtkLabel*) pt_label, TRUE);
	gtk_misc_set_alignment ((GtkMisc*) pt_label, 0.0f, 0.5f);
	gmpc_metadata_browser_add_entry (self, info_box, _ ("Dates"), NULL, (GtkWidget*) pt_label, &i);
	/* Total number of songs */
	_tmp8_ = NULL;
	pt_label = (_tmp8_ = g_object_ref_sink (gmpc_stats_label_new (ALBUM_NUM_SONGS, song)), (pt_label == NULL) ? NULL : (pt_label = (g_object_unref (pt_label), NULL)), _tmp8_);
	gtk_label_set_line_wrap ((GtkLabel*) pt_label, TRUE);
	gtk_misc_set_alignment ((GtkMisc*) pt_label, 0.0f, 0.5f);
	gmpc_metadata_browser_add_entry (self, info_box, _ ("Songs"), NULL, (GtkWidget*) pt_label, &i);
	/* Total playtime */
	_tmp9_ = NULL;
	pt_label = (_tmp9_ = g_object_ref_sink (gmpc_stats_label_new (ALBUM_PLAYTIME_SONGS, song)), (pt_label == NULL) ? NULL : (pt_label = (g_object_unref (pt_label), NULL)), _tmp9_);
	gtk_label_set_line_wrap ((GtkLabel*) pt_label, TRUE);
	gtk_misc_set_alignment ((GtkMisc*) pt_label, 0.0f, 0.5f);
	...
	/* Player controls */
	_tmp10_ = NULL;
	hbox = (_tmp10_ = g_object_ref_sink ((GtkHBox*) gtk_hbox_new (FALSE, 6)), (hbox == NULL) ? NULL : (hbox = (g_object_unref (hbox), NULL)), _tmp10_);
	...
	gtk_box_pack_start ((GtkBox*) hbox, (GtkWidget*) button, FALSE, FALSE, (guint) 0);
	_tmp11_ = NULL;
	button = (_tmp11_ = g_object_ref_sink ((GtkButton*) gtk_button_new_with_mnemonic ("_Replace")), (button == NULL) ? NULL : (button = (g_object_unref (button), NULL)), _tmp11_);
	_tmp12_ = NULL;
	...
	_tmp15_ = NULL;
	label = (_tmp15_ = g_object_ref_sink ((GtkLabel*) gtk_label_new ("")), (label == NULL) ? NULL : (label = (g_object_unref (label), NULL)), _tmp15_);
	gtk_label_set_selectable (label, TRUE);
	_tmp16_ = NULL;
...
static gboolean gmpc_metadata_browser_metadata_box_update_real (GmpcMetadataBrowser* self) {
	...
	song = gmpc_metadata_browser_browser_get_selected_song (self);
	if (song != NULL) {
		GmpcMetadataBrowserHitem _tmp1_ = {0};
		GmpcMetadataBrowserHitem item;
		mpd_Song* _tmp3_;
		const mpd_Song* _tmp2_;
		GtkWidget* view;
		/** Add item to history */
		item = (memset (&_tmp1_, 0, sizeof (GmpcMetadataBrowserHitem)), _tmp1_);
		_tmp3_ = NULL;
		_tmp2_ = NULL;
		item.song = (_tmp3_ = (_tmp2_ = song, (_tmp2_ == NULL) ? NULL : mpd_songDup (_tmp2_)), (item.song == NULL) ? NULL : (item.song = (mpd_freeSong (item.song), NULL)), _tmp3_);
		...
		if (_tmp4_) {
			GmpcMetadataBrowserHitem _tmp5_ = {0};
			GmpcMetadataBrowserHitem item;
			mpd_Song* _tmp6_;
			char* _tmp8_;
			const char* _tmp7_;
			...
			const char* _tmp9_;
			/** Add item to history */
			item = (memset (&_tmp5_, 0, sizeof (GmpcMetadataBrowserHitem)), _tmp5_);
			_tmp6_ = NULL;
			item.song = (_tmp6_ = mpd_newSong (), (item.song == NULL) ? NULL : (item.song = (mpd_freeSong (item.song), NULL)), _tmp6_);
			_tmp8_ = NULL;
			_tmp7_ = NULL;
			item.song->artist = (_tmp8_ = (_tmp7_ = artist, (_tmp7_ == NULL) ? NULL : g_strdup (_tmp7_)), item.song->artist = (g_free (item.song->artist), NULL), _tmp8_);
			_tmp10_ = NULL;
			_tmp9_ = NULL;
			item.song->album = (_tmp10_ = (_tmp9_ = album, (_tmp9_ == NULL) ? NULL : g_strdup (_tmp9_)), item.song->album = (g_free (item.song->album), NULL), _tmp10_);
			...
			if (artist != NULL) {
				GmpcMetadataBrowserHitem _tmp11_ = {0};
				GmpcMetadataBrowserHitem item;
				mpd_Song* _tmp12_;
				char* _tmp14_;
				const char* _tmp13_;
				/** Add item to history */
				item = (memset (&_tmp11_, 0, sizeof (GmpcMetadataBrowserHitem)), _tmp11_);
				_tmp12_ = NULL;
				item.song = (_tmp12_ = mpd_newSong (), (item.song == NULL) ? NULL : (item.song = (mpd_freeSong (item.song), NULL)), _tmp12_);
				_tmp14_ = NULL;
				_tmp13_ = NULL;
				item.song->artist = (_tmp14_ = (_tmp13_ = artist, (_tmp13_ == NULL) ? NULL : g_strdup (_tmp13_)), item.song->artist = (g_free (item.song->artist), NULL), _tmp14_);
	...
	self->priv->update_timeout = (guint) 0;
	return (_tmp15_ = FALSE, artist = (g_free (artist), NULL), album = (g_free (album), NULL), (song == NULL) ? NULL : (song = (mpd_freeSong (song), NULL)), _tmp15_);
}
...
static GtkHBox* gmpc_metadata_browser_history_buttons (GmpcMetadataBrowser* self) {
	...
	if (self->priv->current != NULL) {
		...
		if (self->priv->current->next != NULL) {
			...
			_tmp4_ = self->priv->current->prev != NULL;
		}
		_tmp3_ = _tmp4_;
	} else {
		_tmp3_ = FALSE;
	...
	if (self->priv->current == NULL) {
		...
		_tmp5_ = self->priv->current->next == NULL;
	}
	if (_tmp5_) {
	...
	_tmp6_ = NULL;
	return (_tmp6_ = box, (next_but == NULL) ? NULL : (next_but = (g_object_unref (next_but), NULL)), (back_but == NULL) ? NULL : (back_but = (g_object_unref (back_but), NULL)), _tmp6_);
}
...
void gmpc_metadata_browser_set_song (GmpcMetadataBrowser* self, const mpd_Song* song) {
	GtkTreeIter iter = {0};
	GmpcMetadataBrowserHitem _tmp2_ = {0};
	GmpcMetadataBrowserHitem item;
	mpd_Song* _tmp4_;
	...
	/** Add item to history */
	item = (memset (&_tmp2_, 0, sizeof (GmpcMetadataBrowserHitem)), _tmp2_);
	_tmp4_ = NULL;
	_tmp3_ = NULL;
	item.song = (_tmp4_ = (_tmp3_ = song, (_tmp3_ == NULL) ? NULL : mpd_songDup (_tmp3_)), (item.song == NULL) ? NULL : (item.song = (mpd_freeSong (item.song), NULL)), _tmp4_);
	item.type = GMPC_METADATA_BROWSER_HITEM_TYPE_SONG;
	gmpc_metadata_browser_history_add (self, &item);
	view = gmpc_metadata_browser_metadata_box_show_song (self, song);
	gtk_container_add ((GtkContainer*) self->priv->metadata_box, view);
