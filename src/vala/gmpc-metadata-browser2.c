...
static void gmpc_metadata_browser_metadata_box_clear (GmpcMetadataBrowser* self) {
	GList* list;
	g_return_if_fail (self != NULL);
	list = gtk_container_get_children ((GtkContainer*) self->priv->metadata_sw);
	{
...
static void gmpc_metadata_browser_metadata_box_show_artist (GmpcMetadataBrowser* self, const char* artist) {
	...
	gtk_table_set_row_spacings (info_box, (guint) 8);
	...
	/**
	         * Add it to the view
	         */
	gtk_scrolled_window_add_with_viewport (self->priv->metadata_sw, (GtkWidget*) vbox);
