...
{
    GList *retv = NULL;
    char *temp_b = g_utf8_casefold(song->album,-1);
    xmlDocPtr doc;
    /**
     * Get artist name
     */
                        ...
                        while(cur3){
                            ...
                            if(temp){
                                if(xmlStrEqual(temp, (xmlChar *)"primary"))
                                {
                                    ...
                                    mtd->type = META_ALBUM_ART; mtd->plugin_name = discogs_plugin.name; 
...
static void __query_get_album_art_uris(const GEADAsyncHandler *handle, GEADStatus status, gpointer user_data)
{
    ii *i = (ii *) user_data;
    ...
    if(status == GEAD_DONE) {
        ...
        if(i->releases) {
            ...
            snprintf(furl,1024,DISCOGS_API_ROOT"release%s?f=xml&api_key=%s", &artist_uri[j],DISCOGS_API_KEY);
...
static void __query_get_album_art(const GEADAsyncHandler *handle, GEADStatus status, gpointer user_data)
{
    ii *i = (ii *) user_data;
    if(status == GEAD_PROGRESS) return;
    if(status == GEAD_DONE) {
...
static void __query_get_artist_art_uris(const GEADAsyncHandler *handle, GEADStatus status, gpointer user_data)
{
    ii *i = (ii *) user_data;
    if(status == GEAD_PROGRESS) return;
    if(status == GEAD_DONE) {
...
static void __query_get_artist_art(const GEADAsyncHandler *handle, GEADStatus status, gpointer user_data)
{
    ii *i = (ii *) user_data;
    if(status == GEAD_PROGRESS) return;
    if(status == GEAD_DONE) {
        ...
        if(artist_uri)
        {
            ...
            {
                snprintf(furl,1024,"%s&f=xml&api_key=%s", artist_uri,DISCOGS_API_KEY);
            }else{
                snprintf(furl,1024,"%s?f=xml&api_key=%s", artist_uri,DISCOGS_API_KEY);
            }
...
static void discogs_fetch_get_image(mpd_Song *song,MetaDataType type, void (*callback)(GList *list, gpointer data), gpointer user_data) 
{
...
static const gchar *discogs_get_translation_domain(void)
{
...
gmpcPlugin discogs_plugin = {
    ...
    .get_translation_domain = discogs_get_translation_domain
};
