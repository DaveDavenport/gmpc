...
static void pl3_current_playlist_browser_priority_remove_selected_songs(PlayQueuePlugin * self);
                                        ...
                                        PlayQueuePlugin * self)
{
    ...
    if (GMPC_IS_MPDDATA_MODEL_PLAYLIST(model))
    {
        ...
        {
            gtk_tree_view_scroll_to_cell(GTK_TREE_VIEW(self->priv->pl3_cp_tree), path, NULL, TRUE, 0.5, 0);
        }
    }
}
...
void pl3_current_playlist_destroy(PlayQueuePlugin * self)
{
    ...
    if (self->priv->pl3_curb_tree_ref)
    {
        ...
        if (path)
        {
            if (gtk_tree_model_get_iter
                (GTK_TREE_MODEL(gtk_tree_row_reference_get_model(self->priv->pl3_curb_tree_ref)), &piter, path))
            {
                gtk_list_store_remove(GTK_LIST_STORE(gtk_tree_row_reference_get_model(self->priv->pl3_curb_tree_ref)),
                                      &piter);
            }
    ...
    if (self->priv->pl3_cp_vbox)
    {
        ...
        g_signal_handlers_disconnect_by_func(G_OBJECT(self->priv->pl3_cp_tree),
                                             G_CALLBACK(pl3_current_playlist_column_changed), NULL);
                                                             ...
                                                             PlayQueuePlugin * self)
{
    if (event->button == 3)
    {
        ...
        if(mpd_server_check_command_allowed(connection, "prioid") == MPD_SERVER_COMMAND_ALLOWED) 
        {
            /* priority */
            item = gtk_menu_item_new_with_label(_("Raise priority"));
            gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
            g_signal_connect_swapped(G_OBJECT(item), "activate",
                    G_CALLBACK(pl3_current_playlist_browser_priority_raise_selected_songs), self);
            /* remove priority */
            item = gtk_menu_item_new_with_label(_("Remove priority"));
            gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
            g_signal_connect_swapped(G_OBJECT(item), "activate",
                    G_CALLBACK(pl3_current_playlist_browser_priority_remove_selected_songs), self);
        }
                                                          ...
                                                          PlayQueuePlugin * self)
{
    ...
    } else if (event->keyval == GDK_KEY_i && event->state & GDK_MOD1_MASK)
    {
        ...
        return TRUE;
    } else if (event->state&GDK_MOD1_MASK && event->keyval == GDK_KEY_q) 
    {
        if(mpd_server_check_command_allowed(connection, "prioid") == MPD_SERVER_COMMAND_ALLOWED) 
        {
            pl3_current_playlist_browser_priority_raise_selected_songs(self);
        }
    } else if (event->state&GDK_MOD1_MASK && event->keyval == GDK_KEY_d) 
    {
        if(mpd_server_check_command_allowed(connection, "prioid") == MPD_SERVER_COMMAND_ALLOWED) 
        {
            pl3_current_playlist_browser_priority_remove_selected_songs(self);
        }
...
static void pl3_current_playlist_save_playlist(void)
{
    ...
    do
    {
        ...
        case GTK_RESPONSE_OK:
            ...
            /* get the song-name */
            str = (gchar *) gtk_entry_get_text(GTK_ENTRY((GtkWidget *) gtk_builder_get_object(xml, "pl-entry")));
            ...
            if (strlen(str) != 0 && mpd_check_connected(connection))
            {
                ...
                {
                    gchar *errormsg =
                        g_markup_printf_escaped(_("<i>Playlist <b>\"%s\"</b> already exists\nOverwrite?</i>"), str);
...
static void pl3_current_playlist_status_changed(GmpcConnection * conn, MpdObj * mi, ChangedStatusType what,
                                                PlayQueuePlugin * self)
{
    if (self->priv->pl3_cp_vbox == NULL)
        return;
...
static void pl3_current_playlist_connection_changed(GmpcConnection * conn, MpdObj * mi, int connect,
                                                    PlayQueuePlugin * self)
{
...
static GObject *play_queue_plugin_constructor(GType type, guint n_construct_properties,
                                              GObjectConstructParam * construct_properties)
{
    ...
    self->priv->status_changed_handler =
        g_signal_connect_object(G_OBJECT(gmpcconn), "status-changed", G_CALLBACK(pl3_current_playlist_status_changed),
                                self, 0);
    self->priv->connection_changed_handler =
        g_signal_connect_object(G_OBJECT(gmpcconn), "connection-changed",
                                G_CALLBACK(pl3_current_playlist_connection_changed), self, 0);
    /* quick search */
    self->priv->search_keep_open = FALSE;
...
static void play_queue_plugin_class_init(PlayQueuePluginClass * klass)
{
    ...
    g_object_class_install_property(G_OBJECT_CLASS(klass), PLAY_QUEUE_UID,
                                    g_param_spec_string("uid", "uid", "stores an unique id", "play-queue",
...
static MpdData *play_queue_plugin_is_search(GmpcPluginIntegrateSearchIface * obj, mpd_TagItems num_field,
    ...
    if (list)
    {
        ...
        while(llist)
        {
            ...
            if(gtk_tree_model_get_iter(model, &iter, (GtkTreePath *) llist->data))
            {
                int id;
                gtk_tree_model_get(model, &iter, MPDDATA_MODEL_COL_SONG_ID, &id, -1);
                mpd_playlist_set_priority(connection, id, 0); 
            }
            llist = g_list_next(llist);
        } 
