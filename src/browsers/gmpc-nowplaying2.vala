...
//                        bg.modify_text(Gtk.StateType.NORMAL,this.paned.style.text[Gtk.StateType.NORMAL]);
            ...
            private void update_playing()
			{
				...
				if(song.artist != null) {
					...
					event.set_data("artist",song.artist);
				...
				if(song.album != null) {
					...
					event.set_data("artist",song.artist);
					event.set_data("album",song.album);
					event.button_press_event.connect((widget, event) => {
							string artist = (string)widget.get_data<string>("artist");
...
//					button.add(label);
				...
				ali = new Gtk.Alignment(0.0f, 0.5f,1f,0f);
				ali.add(hboxje);

				/* Create pane in 2. */
				var bottom_hbox = new Gtk.HBox(false, 6);
				/* left pane */
				var metadata_vbox = new Gtk.VBox(false, 0);
				...
				if(song.album != null && song.artist != null)
				{
					...
					if(list != null) {
						...
						do{
							...
							button.set_data("artist",
									but_song.artist.dup());
							button.set_data("album",
									but_song.album.dup());
							stdout.printf("open: %s %s\n", but_song.artist, but_song.album);
							button.clicked.connect((source) => {
									unowned string artist = (string)source.get_data<string>("artist");
									unowned string album = (string)source.get_data<string>("album");
									stdout.printf("open: %s %s\n", artist, album);
									Gmpc.Browser.Metadata.show_album(artist, album);
									});
							albums++;
