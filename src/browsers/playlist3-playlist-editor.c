...
void playlist_editor_browser_add(GtkWidget * cat_tree)
{
	...
	/* Check if enabled */
	if (!cfg_get_single_value_as_int_with_default(config, "playlist-plugin", "enable", TRUE))
		return;
	...
	gtk_list_store_set(store, &iter,
					   ...
					   PL3_CAT_TITLE, _("Playlist Editor"), PL3_CAT_ICON_ID, "media-playlist", -1);
	/**
...
static void playlist_editor_browser_playlist_editor_selected(GtkTreeModel * model, GtkTreePath * path,
			  ...
			  loop:
				if (valid)
				{
					...
					if (val == 0)
					{
						...
						if (path)
						{
							if (gtk_tree_selection_path_is_selected
								(gtk_tree_view_get_selection(GTK_TREE_VIEW(playlist_editor_icon_view)), path))
							{
								if (mtime == NULL || data->playlist->mtime == NULL
									|| strcmp(mtime, data->playlist->mtime) != 0)
								{
									/* update view */
					...
					} else if (val > 0)
					{
						...
						gtk_list_store_set(playlist_editor_store, &niter, PL_NAME, data->playlist->path, PL_MTIME,
										   data->playlist->mtime, PL_IMAGE, pb, -1);
						if (pb)
							g_object_unref(pb);
					} else
					{
				...
				} else
				{
					...
					gtk_list_store_set(playlist_editor_store, &iter, PL_NAME, data->playlist->path, PL_MTIME,
									   data->playlist->mtime, PL_IMAGE, pb, -1);
					if (pb)
						g_object_unref(pb);
...
static void playlist_editor_rename_playlist(GtkWidget * item, gpointer data)
{
	...
	GtkWidget *dialog =
		gtk_dialog_new_with_buttons(_("Rename Playlist"), NULL, GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT,
									GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,
									NULL);
	GtkWidget *button = NULL;
	GtkWidget *hbox = gtk_hbox_new(FALSE, 6);
...
static void playlist_editor_browser_init(void)
{
	...
	g_signal_connect(G_OBJECT(gtk_tree_view_get_selection(GTK_TREE_VIEW(tree))), "changed",
					 G_CALLBACK(playlist_editor_browser_playlist_editor_changed_real), NULL);
//    g_signal_connect(G_OBJECT(tree), "item-activated", G_CALLBACK(playlist_editor_browser_activate_cursor_item), NULL);
	g_signal_connect(G_OBJECT(tree), "button-release-event", G_CALLBACK(playlist_editor_browser_button_release_event),
					 NULL);
...
{
	GtkWidget *item = NULL;
	if (!cfg_get_single_value_as_int_with_default(config, "playlist-plugin", "enable", 1))
		return 0;
