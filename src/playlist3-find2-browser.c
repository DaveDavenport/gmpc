...
/*	gtk_list_store_append(pl3_find2_combo_store, &iter);
	gtk_list_store_set(pl3_find2_combo_store, &iter, 1, "Playlist", 0,PL3_FINDB_CB_PLAYLIST, -1);
...
static void pl3_find2_browser_init()
{
	...
	GValue value = {0,};
	pl3_find2_store = gtk_list_store_new (PL3_FINDB_ROWS, 
	...
	pl3_find2_vbox = gtk_vbox_new(FALSE, 6);
        ...
        /* Add button */
        GtkWidget *addbut = gtk_button_new();
        gtk_button_set_image(GTK_BUTTON(addbut), gtk_image_new_from_stock(GTK_STOCK_ADD, GTK_ICON_SIZE_BUTTON));
        g_signal_connect(G_OBJECT(addbut), "clicked", G_CALLBACK(pl3_find2_browser_add_crit), NULL);
	...
	gtk_label_set_markup(GTK_LABEL(label), _("<span size='xx-large' weight='bold'>Database Search</span>"));
        ...
        gtk_box_pack_start(GTK_BOX(hbox), addbut, FALSE, TRUE,0);       
...
static void pl3_find2_browser_search_playlist(void)
{
   /* if(pl3_find2_tree)
    {
        GtkTreeIter iter;
        gtk_tree_model_get_iter_first(GTK_TREE_MODEL(pl3_find2_combo_store), &iter);
        do{
            int id=0;
            gtk_tree_model_get(GTK_TREE_MODEL(pl3_find2_combo_store), &iter, 0, &id, -1);
            if(id == PL3_FINDB_CB_PLAYLIST)
            {
                gtk_combo_box_set_active_iter(GTK_COMBO_BOX(pl3_find2_combo), &iter);
            }
        }while(gtk_tree_model_iter_next(GTK_TREE_MODEL(pl3_find2_combo_store), &iter));

        gtk_widget_grab_focus(pl3_find2_entry);
    }
*/
}
...
static void pl3_playlist_search()
{
    ...
    if(pl3_xml)
    {
        ...
        gtk_tree_path_free(path);
        pl3_find2_browser_search_playlist();
    }
}
