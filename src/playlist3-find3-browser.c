...
/*	gtk_list_store_append(pl3_find3_combo_store, &iter);
	gtk_list_store_set(pl3_find3_combo_store, &iter, 1, "Playlist", 0,PL3_FIND3_CB_PLAYLIST, -1);

	gtk_combo_box_set_active(GTK_COMBO_BOX(pl3_find3_combo), 0);
*/
}
...
static void pl3_find3_browser_init()
{
    ...
    gtk_tree_view_column_set_attributes (column,renderer,"stock-id", PL3_FIND3_ICON,NULL);
    ...
    gtk_tree_view_column_set_attributes (column,renderer,"text", PL3_FIND3_TITLE, NULL);


    /* set up the tree */
    pl3_find3_tree= gtk_tree_view_new_with_model(GTK_TREE_MODEL(pl3_find3_store));
    ...
    gtk_box_pack_end(GTK_BOX(pl3_find3_vbox), pl3_find3_sw, TRUE, TRUE,0);
...
static void pl3_find3_browser_show_info()
{
    ...
    if (gtk_tree_selection_count_selected_rows (selection) > 0)
    {
        ...
        do
        {
            ...
            gtk_tree_model_get (model, &iter, PL3_FIND3_TYPE, &type,-1);
            if(type == PL3_CUR_PLAYLIST)
            {
                gtk_tree_model_get(model,&iter,PL3_FIND3_PID, &id,-1);
                call_id3_window (id);
            }
            else
            {
                mpd_Song *song =NULL;
                char *path;
                gtk_tree_model_get(model,&iter,PL3_FIND3_PATH, &path,-1);
...
static void pl3_find3_browser_button_release_event(GtkWidget *but, GdkEventButton *event)
{
    ...
    else if(gtk_tree_selection_count_selected_rows(gtk_tree_view_get_selection(GTK_TREE_VIEW(pl3_find3_tree))) > 0)
    {
        ...
        /*       item = gtk_image_menu_item_new_from_stock(GTK_STOCK_ADD,NULL);
                 ...
                 G_CALLBACK(pl3_find3_browser_add_selected), NULL);
        ...
        /*
           ...
           gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
           g_signal_connect(G_OBJECT(item), "activate",
           G_CALLBACK(pl3_find3_browser_replace_selected), NULL);
           */
        if(mpd_server_check_version(connection,0,12,0))
        {
...
static int pl3_find3_browser_add_go_menu(GtkWidget *menu)
{
    ...
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
    g_signal_connect(G_OBJECT(item), "activate", 
            G_CALLBACK(pl3_find3_browser_activate), NULL);
