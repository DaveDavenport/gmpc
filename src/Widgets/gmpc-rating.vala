...
public class Gmpc.Rating : Gtk.Frame
{
    ...
    ~Rating() {
        if (this.status_changed_id > 0 &&  GLib.SignalHandler.is_connected(gmpcconn,
        this.status_changed_id)) {
    ...
    public Rating (MPD.Server server, MPD.Song song) {
        ...
        this.song = song.copy();
        this.update();
        this.status_changed_id = GLib.Signal.connect_swapped(gmpcconn, "status_changed",
        (GLib.Callback)status_changed, this);
    }
    ...
    public void update()
    {
        ...
        if(value == null) {
            ...
            this.set_rating(value.to_int()/2);
        }
    }
}
